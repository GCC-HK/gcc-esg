...
Owner iterations after go-live: Tools nav dropdown + tools overview page (CBAM live again, CN-gated; 3 pipeline tools) · Regulations/Deadlines split into dedicated pages · focused subpages for FAQ, Glossary (term/definition rows incl. EU law-types explainer) and Voluntary Certifications; Learn nav is a dropdown · legal topline moved below header · hub decluttered (no step chips, badges as pills on card frames, deadlines mini full width, trust/disclaimer boxes off the hub) · wizard multi-category selection → comparison matrix (auto-advance removed, pane-1 Next restored) · CBAM volume period selector (year/quarter/shipment; annualised 50 t check) per Jill's #25 · persona glow always on · serif retired platform-wide (`--font-serif` → sans) · no dash punctuation in platform copy (test-enforced; CMS content still pending re-seed). Round 4 (same day): persona glow without active outline · Regulations nav split (Suppliers: guided check / Sourcing Offices: express table, one finder variant visible per perspective + on-page mode switch) · CBAM price-basis note light-on-dark · briefing lead full width image-left/text-right, smaller posts columned · Learn dropdown reordered (Understand & Plan, certifications, FAQ last) · certifications and glossary as name-left/explanation-right rows, glossary gained a 'What that means for you' action column · nav About renamed Our Team.

## Phase 15 — News refresh prep, briefing news layout, brand completion (5 Sep 2026)

**Briefing page** rebuilt as a classic news site per owner: lead story full width (image fills exactly the left grid cell beside the text — absolute fill inside the cell after a grid sizing bug made the panorama overlap the text), all other posts as uniform rows (180px thumb, kicker, headline, 3-line clamped teaser; What-to-do trimmed from compact rows). Regulations dropdown items renamed **Guided Check / Express Lookup**.

**Content review (5 Sep):** all 12 CMS posts fact-checked via web research against official/legal sources — nothing wrong, four stale. `scripts/seeds/seed-news-sep26.js` prepared (NOT yet run — needs SANITY_TOKEN): new lead "Three weeks to the EU greenwashing ban" (Dir. 2024/825 applies 27 Sep 2026), ESPR steel DA expected Q4 2026 + DPP registry, China supply-chain security regs (Apr 2026) vs EU due diligence, Toy Safety Reg 2025/2509; updates to CBAM-457 (Council position 12 Jun, plenary this month), PPWR (now applied: Art. 5 + DoC), CBAM explainer (declaration deadline now 30 Sep 2027), ETS (EPRS stage); plus a dynamic dash-cleanup pass over all newsPost + deadline text.

**Dead-link and identity cleanup:** the never-existing gcc-sustainability@ address was still on regulation/account/article/privacy pages (16× on privacy) — all now info@hongkong.ahk.de; privacy ZH said 19/F, corrected to 23/F; "Get the weekly briefing" (pointed at the removed V1 subscribe form) and "Ask the Committee" deleted from regulation detail pages per owner (only "Read the official text" remains); briefing subscribe button now mails the office. **Brand sweep completed**: GREEN FUTURE remnants purged from legacy pages, the CBAM print footer (which also carried the old committee name + dead email), ICS PRODID, subscribe email strings, dev server, SETUP.md — repo greps clean.
