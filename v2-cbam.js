...
(function () {
    ...
    const DISC = `<div class="v2-disclaimer"><span aria-hidden="true">&#9888;</span><div><span class="lang-en"><strong>Indicative estimate, not a CBAM declaration.</strong> Official prices, benchmarks and country default values are applied where available, but your actual liability depends on exact CN codes and verified installation data. Method notes below.</span><span class="lang-zh"><strong>指示性估算，并非CBAM申报。</strong>已尽可能采用官方价格、基准值和国家默认值，但实际义务取决于具体CN编码和经核实的设施数据。方法说明见下方。</span><span class="lang-de"><strong>Indikative Sch&auml;tzung, keine CBAM-Erkl&auml;rung.</strong> Offizielle Preise, Benchmarks und L&auml;nder-Standardwerte werden angewendet; die tats&auml;chliche Belastung h&auml;ngt von CN-Codes und verifizierten Anlagendaten ab.</span><span class="lang-vi"><strong>Ước t&iacute;nh tham khảo, kh&ocirc;ng phải khai b&aacute;o CBAM.</strong> Đ&atilde; &aacute;p dụng gi&aacute;, chuẩn v&agrave; gi&aacute; trị mặc định ch&iacute;nh thức khi c&oacute;; nghĩa vụ thực tế phụ thuộc m&atilde; CN v&agrave; dữ liệu cơ sở đ&atilde; x&aacute;c minh.</span></div></div>`;
